#inspired from https://github.com/alexiglad/EBT/blob/main/model/nlp/ebt.py
import sys
import os
...
class Lyapunov(nn.Module):
    def __init__(self, input_dim, hidden_dim, epsilon=0.01):
        ...
        # Standart bir Feed-Forward Ağ (g(x))
        self.net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.Tanh(),  # Tanh veya ReLU/Leaky ReLU kullanılabilir
            nn.Linear(hidden_dim, 1) # Çıktı skaler olmalı
        )
        
        # Ağı denge noktasında (x=0) sıfıra merkezlemek için
        # x=0'da V(x)=0 koşulunu sağlamaya yardımcı olur
        self.g_0 = self.net(torch.zeros(1, input_dim)) 

    def forward(self, x):
        
        # Nöral ağ çıktısı (g(x))
        g_x = self.net(x).to(device)
        
        # V(x) = (g(x) - g(0))² + epsilon * ||x||²
        # Skaler çıktıları kare alarak ve ||x||^2 ekleyerek pozitif kesinlik sağlanır.
        # Bu, V(x) > 0 (x!=0) ve V(0)=0 şartını garanti etmenin yaygın bir yoludur.
        V_x = torch.sum(torch.pow(g_x - self.g_0.to(device), 2), dim=1, keepdim=True) + self.epsilon * torch.sum(torch.pow(x, 2), dim=1, keepdim=True)
        
        return V_x
    
def lyapunov_derivative(V_net, x, f_x):
    """
    Lyapunov fonksiyonunun türevini (Lie Türevi) hesaplar: V_dot = ∇V(x) * f(x)
    """
    # x için türev hesaplayabilmek adına gradyenleri etkinleştir.
    x.requires_grad_(True)
    # 1. V(x) değerini hesapla
    V_x = V_net(x)

    # 2. V(x)'in x'e göre gradyanını (∇V) hesapla
    # create_graph=True, ikinci türevleri (Hessian vb.) hesaplamak için gerekliyse kullanılır.
    # Bu örnek için gerek yok, ancak karmaşık kayıp fonksiyonlarında önemlidir.
    grad_V = torch.autograd.grad(V_x, x, grad_outputs=torch.ones_like(V_x), create_graph=True)[0]
    
    # 3. Lie Türevini hesapla: V_dot = ∇V * f(x)
    V_dot = torch.sum(grad_V * f_x, dim=1, keepdim=True)
    
    return V_dot

def lyapunov_loss(V_net, f_dynamics, samples,B,T, margin=0.01):
    """
    Lyapunov Kayıp Fonksiyonu.
    Pozitif Kesinlik Kaybı (Genellikle mimari ile halledilir) + Azalma Kaybı
    """ 
    if(len(samples.shape)!=3):
        samples=rearrange(samples,"(B T) 1 -> B T",B=B)

    
    # 1. Dinamik sistemi hesapla: f(x)
    f_x = f_dynamics(samples,T)
    
    # 2. Lyapunov türevini (V_dot) hesapla: ∇V(x) * f(x)
    V_dot = lyapunov_derivative(V_net, samples, f_x)
    
    # 3. Azalma Koşulu Kaybı (V_dot < 0)
    # Koşul: V_dot <= -margin
    # İhlal (Loss): max(0, V_dot + margin)
    
    # x'in 0'a yakın olmadığı bölgelerde kararlılığı zorla
    # Denge noktası (x=0) yakınında V_dot'un negatife gitmesi kararlılık için kritiktir.
    # Genellikle denge noktasından belirli bir mesafedeki örnekler için bu kayıp hesaplanır.
    
    # V_dot'un 0'dan büyük (yani koşulu ihlal eden) kısımlarını cezalandır.
    # Ayrıca V_dot'un pozitif olan kısımlarını, bir marjin (-margin) kadar daha negatif olmaya zorla.
    loss_stability = torch.mean(torch.pow(F.relu(V_dot.squeeze() + margin), 2))

    # Sadece kararlılık kaybını kullanıyoruz. Pozitif kesinlik V_net mimarisi tarafından garanti ediliyor.
    return loss_stability


def system_dynamics(x,INPUT_DIM):
    """
    Dinamik sistemin vektör alanını (f(x)) tanımlar.
    x: [B, 127] boyutunda.
    Çıktı da [B, 127] boyutunda olmalıdır (∇V ile uyumlu olması için).
    """
    # 1. Lineer Kısım (Kararlılığı sağlayan A matrisi)
    # Varsayım: A matrisi denge noktası etrafında kararlıdır.
    A = torch.eye(INPUT_DIM, device=x.device) * -0.5 
    linear_part = x @ A
    
    # 2. Nolineer Kısım (Örnek olarak bir sinüs fonksiyonu)
    # Her bir boyuta nolineer bir etki eklenir.
    nonlinear_part = 0.1 * torch.sin(x) 
    
    # Dinamik sistem: f(x)
    dx = linear_part + nonlinear_part
...
class EBT_NLP(L.LightningModule):
    def __init__(self, hparams,config_dict:Dict):
        ...
        self.tokenizer_pad_token = tokenizer.eos_token # is token 0, was right padding things
        self.tokenizer_pad_token_id=5
        ...
        self.lyap=Lyapunov(self.hparams.model_max_length-1,self.hparams.model_max_length-1//2).to(device)
        self.V_prev = float('inf')
        self.P=torch.eye(self.hparams.embedding_dim//2)

        self.step_step_controller = AdaptiveStepSizeController(
    ...
    def forward(self, x,carry, start_pos = 0, learning = True, return_raw_logits = False, replay_buffer_logits = None, no_randomness = True,past_cache=None,prev_energy=None): # accepts input_ids as input; a lot of the logic here is just for S2 params, see pseudocode in paper for a more concise view of how this works. it can be < 10 LOC
        ...
        real_embeddings_input = self.embeddings(x) #B,T-1
        batch_size = x.shape[0] 
        seq_length = x.shape[1]

        alpha = torch.clamp(self.alpha, min=0.0001)
        if not no_randomness and self.hparams.randomize_mcmc_step_size_scale != 1:
            expanded_alpha = alpha.expand(batch_size, seq_length, 1)#B,T-1

            scale = self.hparams.randomize_mcmc_step_size_scale
            low = alpha / scale
            high = alpha * scale
            alpha = low + torch.rand_like(expanded_alpha,requires_grad=True) * (high - low)

        langevin_dynamics_noise_std = torch.clamp(self.langevin_dynamics_noise_std, min=1e-6)
        ...
        with torch.set_grad_enabled(True):
            for i, mcmc_step in enumerate(mcmc_steps):
                ...
                grad_norm = torch.norm(predicted_tokens_grad.reshape(batch_size, -1), p=2, dim=1).mean()
                
                # İlk iterasyonda previous_energy yoksa mevcut energy'yi kullan
                if prev_energy is None:
                    previous_energy = energy_preds.detach()
                
                # Adaptive alpha hesapla
                current_energy_reshaped = energy_preds.reshape(batch_size, -1).mean(dim=1)
                previous_energy_reshaped = previous_energy.reshape(batch_size, -1).mean(dim=1)
                
                adaptive_alpha = self.adaptive_step_controller(
                    current_energy=current_energy_reshaped,
                    previous_energy=previous_energy_reshaped,
                    gradient_norm=grad_norm,
                    step_idx=i,
                    total_steps=len(mcmc_steps)
                )
                
                # Alpha'yı güncelle (broadcast için reshape)
                if adaptive_alpha.dim() == 0:  # scalar ise
                    alpha = adaptive_alpha.unsqueeze(0).unsqueeze(0).unsqueeze(0)
                else:
                    alpha = adaptive_alpha.view(batch_size, 1, 1)
                
                # Previous energy'yi güncelle
                previous_energy = energy_preds.detach()
            else:
                # Mevcut sabit alpha kullan (kodundaki mevcut mantık)
                pass
    ...
    def forward_loss_wrapper(self, x, phase="train",carry=None,past_cache:List=None,prev_energy=None):
        ...
        predicted_energy=rearrange(predicted_energy,"(B T) 1 -> B T 1",B=B)
        if(phase=="inference"):
            return predicted_energy,new_cache
    ...
    """    def lyapunov_loss(self,x,alpha):
        V_prev=self.V_prev
        B=x.shape[0]
        grad = torch.autograd.grad(torch.sum(x ** 2), x, create_graph=True)[0]
        x = (x - alpha * grad).detach().requires_grad_()

        V = calculate_lyapunov(x, self.P.to(device))
        if torch.any(V > self.V_prev):
            print(f"Lyapunov arttı!  V_max={V.max():.4f}  önceki={V_prev:.4f}")
        self.V_prev = V.max().item()
        u = torch.zeros(B, 1)
        V_dot = self.lyap.lyapunov_derivative(x, u, self.lyap.f, self.lyap.g, self.P)
        return V_dot"""
    
    def calculate_contrastive_loss(self, predicted_energies, input_ids, next_token_indices,past_cache):
        ...
        real_energies,new_cache = self.transformer(all_true_embeddings, start_pos=0, mcmc_step=self.hparams.mcmc_num_steps - 1,past_cache_list=past_cache) # NOTE if want to use this maybe check in better detail what ired does
        ...
        return contrastive_loss, new_cache
...
if __name__=="__main__":
    ...
    ÇIKTI=tokenizer("SENİ SEVECEĞİMİ HİÇ DÜŞÜNMEMİŞTİM BİLE", padding="max_length", truncation=True,return_tensors="pt",max_length=128).to(device)
    ...
    out=model.forward_loss_wrapper(ÇIKTI,phase="train")
    #out=model.ebt_advanced_inference(ÇIKTI["input_ids"],start_pos=0)
    for i in out:

        print(i.shape,"huhuhuhu")
    print("tamamlandı")
