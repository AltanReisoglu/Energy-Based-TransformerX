...
def lyapunov_loss(V_net, f_dynamics, samples, B, T, margin=0.01):
    ...
    samples: predicted_energy tensörü
    B: Batch size
    T: Sequence length
    """
    # samples'ı doğru şekle getir
    # Gelen shape: [B*T, 1] veya [B, T, 1]
    if samples.dim() == 2:  # [B*T, 1] -> [B, T]
        samples = samples.view(B, T)
    elif samples.dim() == 3:  # [B, T, 1] -> [B, T]
        samples = samples.squeeze(-1)
    
    print(f"lyapunov_loss - samples shape after reshape: {samples.shape}")  # [B, T]
    
    # 1. Dinamik sistemi hesapla: f(x)
    f_x = f_dynamics(samples, T)  # [B, T]
    
    # 2. Lyapunov türevini (V_dot) hesapla
    V_dot = lyapunov_derivative(V_net, samples, f_x)  # [B, 1]
    
    # 3. Azalma Koşulu Kaybı
...
def lyapunov_derivative(V_net, x, f_x):
    """
    Lyapunov fonksiyonunun türevini hesaplar
    
    x: [B, T] boyutunda
    f_x: [B, T] boyutunda
    """
    # x için gradyan hesaplayabilmek için
    if not x.requires_grad:
        x = x.clone().requires_grad_(True)
    
    print(f"lyapunov_derivative - x shape: {x.shape}")  # [B, T]
    
    # 1. V(x) değerini hesapla
    V_x = V_net(x)  # [B, 1]
    print(f"lyapunov_derivative - V_x shape: {V_x.shape}")

    # 2. V(x)'in x'e göre gradyanını hesapla
    ...
    print(f"lyapunov_derivative - grad_V shape: {grad_V.shape}")
    
    # 3. Lie Türevini hesapla: V_dot = ∇V * f(x)
    V_dot = torch.sum(grad_V * f_x, dim=1, keepdim=True)  # [B, 1]
...
class Lyapunov(nn.Module):
    def __init__(self, input_dim, hidden_dim, epsilon=0.01):
        ...
        # x=0'da V(x)=0 için referans değer
        with torch.no_grad():
            self.register_buffer('g_0', self.net(torch.zeros(1, input_dim)))

    def forward(self, x):
        """
        x: [B, input_dim] = [B, 127] boyutunda olmalı
        """
        if x.dim() != 2:
            raise ValueError(f"Lyapunov forward expects 2D input [B, input_dim], got shape {x.shape}")
        ...
        V_x = torch.sum(torch.pow(g_x - self.g_0, 2), dim=1, keepdim=True) + \
              self.epsilon * torch.sum(torch.pow(x, 2), dim=1, keepdim=True)
...
def system_dynamics(x, INPUT_DIM):
    ...
    Önemli: x tensörünün shape'i [N_SAMPLES, INPUT_DIM] olmalıdır.
    N_SAMPLES = B * T. INPUT_DIM = 127. (Örnek: [381, 127])
    """
    
    # 1. Giriş Boyutunu (127) doğru yakala
    
    
    # 2. x.unsqueeze(-1) satırı önceki hataya yol açıyordu, bu yüzden KALDIRILMALI.
    #    x tensörü matris çarpımı için zaten [N_SAMPLES, INPUT_DIM] olarak hazırdır.
    
    x=torch.squeeze(x,dim=-1)
    # 3. Lineer Kısım (A matrisi [127, 127] olmalıdır, BATCH boyutu olmamalıdır.)
    A = torch.eye(INPUT_DIM, device=x.device) * -0.5 
    # A'nın boyutu: [127, 127]
    print(A.shape,"maskdmkasmsadk")
    print(x.shape,"asdasd")
    # Matris Çarpımı: [N_SAMPLES, 127] @ [127, 127] -> [N_SAMPLES, 127]
    # Bu, her bir N_SAMPLES durumu için aynı A matrisi ile çarpım yapar.
    linear_part = torch.matmul(x,A) 
    
    # 4. Nolineer Kısım (Element bazlı çarpım)
    nonlinear_part = 0.1 * torch.sin(x) 
    
    # Dinamik sistem: f(x)
    dx = linear_part + nonlinear_part
    print("hahahahaeqwewqewqwqwqwqwqwqwqwqwqwqwqwqwqwqwq")
    return dx 
...
class EBT_NLP(L.LightningModule):
    ...
    def forward(self, x, start_pos = 0, learning = True, return_raw_logits = False, replay_buffer_logits = None, no_randomness = True,past_cache=None,prev_energy=None): # accepts input_ids as input; a lot of the logic here is just for S2 params, see pseudocode in paper for a more concise view of how this works. it can be < 10 LOC
        ...
        with torch.set_grad_enabled(True):
            for i, mcmc_step in enumerate(mcmc_steps):
                ...
                print(energy_preds.shape,"ben denizde bir gemi")
    ...
    def forward_loss_wrapper(self, x, phase="train",past_cache:List=None,prev_energy=None):
        ...
        print(predicted_energies[0].shape,"madingdingdingdindunnn")
    ...
    def _run_ebt_inference_steps(
        ...
        def do_mcmc_step(step_idx, cur_pred_tokens, alpha):
            with torch.set_grad_enabled(True):
                ...
                if self.hparams.infer_accept_lower_energies: # have to get energy to determine if should decrease
                    ...
                    new_energies,new_cache = get_energy(step_idx, proposed_tokens,past_cache=past_cache).reshape(cur_pred_tokens.shape[:2])
        ...
        def get_energy(step_idx, cur_pred_tokens): # for if just want to get energy of currently predicted tokens
            with torch.no_grad():
                ...
                energies,new_cache = self.transformer(combined_embeddings, start_pos=start_pos, mcmc_step=step_idx,past_cache_list=past_cache)
                energies = energies.reshape(-1)
                print(energies.shape,"mfdksdmfksfdmksfdöm")
                import sys;sys.exit()
        ...
        final_pred_state_energies,new_cache = get_energy((self.hparams.mcmc_num_steps - 1), pred_state)
...
if __name__=="__main__":
    ...
    out=model.forward_loss_wrapper(ÇIKTI,True)
    print(out[-1].shape)
    print("tamamlandı")
